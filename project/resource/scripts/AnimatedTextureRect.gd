...
func play(animation_name: String = current_animation):
	...
	get_animation_data(current_animation)
	texture = atlas_to_image_texture(sprites.get_frame_texture(current_animation,frame_index))
	playing = true
...
func get_next_frame():
	...
	if frame_index >= frame_count:
		...
		if not sprites.get_animation_loop(current_animation):
			playing = false
	get_animation_data(current_animation)
	return atlas_to_image_texture(sprites.get_frame_texture(current_animation,frame_index))

func atlas_to_image_texture(atlasTexture: AtlasTexture):
	var image_texture := ImageTexture.new()
	image_texture.set_image(atlasTexture.atlas.get_image().get_region(atlasTexture.region))
