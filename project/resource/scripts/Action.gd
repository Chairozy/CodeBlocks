...
class ControlStep:
	...
	func _init(p_step: int, p_is_loop: bool, p_is_else: bool, _bni: int):
		begin_step = p_step
	...
	func hightlight():
		if bni:
			instance_from_id(bni).hightlight(0.5)
...
func exec():
	...
	return index < steps.size() or runtime_controls.size() > 0

func run_step():
	if index < steps.size():
		var block_node = instance_from_id(steps[index].bni)
		if block_node:
			block_node.hightlight()
		await steps[index].act.call()
		if steps[index].bni:
			block_node.offlight()
		index += 1
	...
	while rcs > 0:
		if runtime_controls[rcs - 1].end_step == index:
			if runtime_controls[rcs - 1].expression.call():
				runtime_controls[rcs - 1].hightlight()
...
func add(act: Callable, bni = null):
	steps.append({"act":act, "bni":bni})
	creating_step += 1

func add_control(expression: Callable, is_loop_control: bool = false, is_else_control: bool = false, bni = null):
	var cs := ControlStep.new(creating_step, is_loop_control, is_else_control, bni)
